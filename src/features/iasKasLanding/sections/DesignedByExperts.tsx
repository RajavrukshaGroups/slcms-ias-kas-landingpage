...
export default function DesignedByExperts() {
  ...
  return (
    <section id="experts" className="py-16 sm:py-20 bg-premium-light bg-arch-watermark border-b border-[#E5E1D8] relative overflow-hidden" ref={ref}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <h2 className="font-sans text-[30px] sm:text-[38px] lg:text-[48px] font-bold leading-[1.1] tracking-tight text-[#082B50]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 items-stretch">
          {expertPillars.map((pillar, idx) => {
            ...
            return (
              <div
                key={pillar.title}
                className={`bg-white/95 backdrop-blur-xs rounded-2xl p-6 sm:p-8 border border-[#E5E1D8] shadow-xs hover-premium-card transition-all duration-700 ease-out flex flex-col justify-between h-full ${isInView ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-95'
                  }`}
